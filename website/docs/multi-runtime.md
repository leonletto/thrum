...
description: "Run Claude, Codex, Aider, Gemini, and other agent runtimes in tmux-managed sessions"
...
A runtime is the CLI you're running inside a tmux pane. Not the agent — the
tool the agent uses to think and type. When Thrum launches a session, it starts
one of these:
...
a follow-up to another implementer on Codex. The messaging layer doesn't know
or care about runtimes.
...
Pass `--runtime` when you first set up a worktree. Thrum writes the value to
the identity file as `preferred_runtime`.
...
and writes it to the `runtime` field in the identity file. This is the
*what's actually running* field — distinct from `preferred_runtime`, which is
the *what should run here* field.
...
step and logs a note. The agent relaunches clean, without its prior conversation.
...
This is step 3 in the resolution chain. Any worktree without a `preferred_runtime`
in its identity file uses this value. Any worktree with `preferred_runtime` set
ignores it.
