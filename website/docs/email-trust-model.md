...
description:
  "How v0.11 scopes trust to mailbox access, what is and isn't signed,
  what's deferred to v0.11.x, and the rotation procedure for suspected compromise"
category: "email"
order: 6
...
Understanding what v0.11 guarantees — and what it doesn't — before deploying
the email bridge in a multi-operator mesh.
...
claim. Any party with access to the outbound SMTP relay can set this header
...
2. **Revoke all app passwords for the compromised account.** At the provider's
   security settings, delete every app password associated with the account
   (not just the Thrum one).

3. **Generate a new app password** and note it.

4. **Update each daemon's credentials:**

   ```bash
   thrum email init --address thrum-mesh@example.com --update-credentials
   ```

   Run this on every daemon that uses the compromised mailbox credentials.
...
6. **Verify the mesh re-converges.** After all daemons restart, run
   `thrum email list` on each side. Existing pairing entries are preserved —
   no re-pair is needed. The daemons re-establish contact through the new
...
- **Prompt rotation** — reduce the blast radius of a compromise by rotating
  app passwords on a schedule (e.g. every 90 days), even without a suspected
  incident.
