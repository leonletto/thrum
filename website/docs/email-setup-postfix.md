...
description:
  "Configure the Thrum email bridge against a self-hosted Postfix MTA —
  STARTTLS auth, daemon-handle naming, and common Postfix pitfalls"
category: "email"
order: 4
...
The handle is set with `--display-name` during `thrum email init`. It becomes
...
thrum email init \
  ...
  --smtp-port 587 \
  --display-name "daemon-a"
```

The command prompts for the mailbox password and writes
...
Verify the SASL password is correct. Check that `smtpd_sasl_auth_enable = yes`
is set in `main.cf` for the submission daemon, not just the main smtpd.

**`454 4.7.0 TLS not available`**
Postfix TLS certificates are missing or expired. Check `/var/log/mail.log` for
OpenSSL errors and renew the certificate.

**IMAP connects but SMTP rejects**
Postfix `smtpd_recipient_restrictions` may not include
`permit_sasl_authenticated`. Verify `master.cf` has the restriction set on the
submission service specifically.

**`Connection refused` on port 587**
The submission service may not be enabled. Check `master.cf` for the
`submission` line and restart Postfix after changes:
...
For a closed private mesh (all daemons on the same domain or LAN), SPF/DKIM
are not required — authenticated submission over STARTTLS is sufficient.
