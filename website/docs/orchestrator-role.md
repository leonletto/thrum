...
description: "Hand off a plan you wrote and let the orchestrator run the execution phase"
...
do. That's intentional. See [Tmux-Managed Sessions](tmux-sessions.md) for how
to get it running.
...
whole point of working this way is that you understand what got built — you can't
do that if code appears on main while you were away.
...
differently when the orchestrator asks. It negotiates per-run, not once globally.
...
When an agent hits a review gate, it sends the orchestrator a completion
message and stops. The orchestrator dispatches a code review sub-agent. The
sub-agent diffs the work against the spec, reports findings, and the
orchestrator either clears the agent to continue or sends findings back for
fixes.

At `per_epic` autonomy, the orchestrator also surfaces the review results to
you before proceeding. At `end_only`, it continues automatically if review
passes.
...
- **Review blockers after 3 rounds** — sends you the review findings and the
  agent's three attempts to fix them, asks whether to continue with exceptions or
  stop.
- **Dependency conflict between epics** — two parallel epics touch overlapping
  code in a way that will conflict at merge time.
- **Judgment call in the plan** — any ambiguity it can't resolve from the
  plan file alone.
...
**`default_autonomy`** — pre-selects the answer to the autonomy question at
plan start. Values: `per_epic`, `end_only`. Default: `end_only`. You can
override at runtime — the orchestrator always confirms before executing.

**`merge_target`** — the branch all agent branches merge to when you approve
the final report. Default: `main`. The orchestrator confirms this branch with
you on first activation. Change it here if you're working on a release branch
or a long-lived integration branch.

The `worktrees` key is also relevant — the orchestrator uses it to know where
to create agent worktrees:
...
The orchestrator preamble lives at `toolkit/templates/roles/orchestrator.md`
in the Thrum repo. It's the context file the orchestrator loads at startup via
...
See [CLI Reference](cli.md) for the full command reference, including `thrum
tmux` subcommands for session lifecycle management.
...
merge report and a question. You answer it. This isn't configurable — there's
no autonomy level that skips the merge gate. The human always controls what
lands on the merge target.
...
- [CLI Reference](cli.md) — full reference for `thrum tmux`, `thrum agent
  set-status`, `thrum worktree`, and other commands
