...
title: "Email Bridge"
description:
  "Route Thrum messages across machines and repositories over IMAP/SMTP —
  no dedicated server required, works with Gmail, Fastmail, iCloud, or
  self-hosted Postfix"
category: "email"
order: 0
...
IDLE). It's not real-time, but it's reliable, asynchronous, and works across
any network.
...
- **Inbound:** the IMAP IDLE goroutine wakes on new mail, parses the
  `X-Thrum-From-Daemon` header to identify the sender, and routes the payload
  to the local daemon's message store
