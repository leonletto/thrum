...
When you need multi-agent coordination, toggle it on: `thrum single-agent-mode
...
context across sessions, that pattern is now built into Thrum. Three files, three
purposes, clean separation. And `thrum prime` now delivers all of it in one
call — no more two-step "run prime, then run context show."
...
`thrum init` now defaults to `single_agent_mode: true` in
`.thrum/config.json`. In this mode:
...
- **Run multiple agents in parallel** — a coordinator on `main` and
  implementers on worktree branches, messaging each other as they work
...
That's it. Next time `thrum prime` runs, it includes the messaging protocol
and listener spawn instructions. The cron watchdog and stop hook activate
...
The listener stops getting spawned, the stop hook exits early, and the
messaging protocol drops out of your preamble. Everything is read at runtime —
no files get rewritten in either direction.
...
halfway through the auth refactor") gets mixed in with project-level stuff
("we use SQLite for the event store") and you end up with a document that's
either stale or bloated.
...
daemon status, then tell you to run `thrum context show` as a separate step.
Now it delivers the complete briefing — identity, role instructions, project
state, and session context — in a single call. The agent is oriented and ready
to work immediately.
...
evolves. For the full technical details on the three-tier model, file paths,
and update mechanics, see the [Context Management](context.md) docs.
...
already exists and doesn't have `single_agent_mode` set, so the daemon treats
it as `false` (multi-agent). Everything works exactly as before. The listener
...
state document manually, you don't lose that work. Ask your agent to look at
the new project state format (`/thrum:update-project`) and import your
existing content into it. From then on, Thrum maintains it for you.

---

## What's Next

Right now, Thrum coordinates agents within a single repo. That covers most
workflows. But if you work with multiple related repos — a primary service and
a couple of microservices, or a monorepo with satellite projects — you've
probably hit the wall where a change in one repo requires a corresponding
change in another. Today you handle that manually: figure out the dependency,
switch to the other repo, explain the context to a different agent, and hope
you don't miss anything.

Cross-project agent coordination shipped in the same release. The approach
turned out simpler than daemon-to-daemon communication — a shared Telegram
group acts as the transport layer between repos, no network configuration
needed. Your agent in the primary repo makes a breaking API change and sends a
message to the other repo's coordinator through the Telegram group. The agents
coordinate directly instead of you relaying the context manually.

See [Cross-Repo Communication](telegram-groups.md) for the full setup.
