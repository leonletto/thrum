...
| `10-snapshot-check-cli` | `thrum tmux snapshot check` returns exit 0 when a snapshot file exists for the calling agent. Asserts the success branch of the check contract (migrates § 10G.2). |
| `11-snapshot-restore-cli` | `thrum tmux snapshot restore` outputs the snapshot markdown to stdout (matched by scenario 09's reason marker) AND consumes the file. A follow-up `thrum tmux snapshot check` then exits 1 — pinning the inverse branch of the check contract (migrates § 10G.3). |
