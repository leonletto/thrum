...
| `17-context-update-project` | NL "update my thrum context with: <marker>" round-trips through claude → `thrum context save` → `thrum context show --session`, asserting the marker comes back via `! thrum context show --session` from the COORD pane. Covers the user-facing path § 9.1 documents (deviation: spec asserts free-form chat prose; we assert the underlying CLI contract for determinism). |
| `18-context-show-saved` | Out-of-pane (`tmux-exec` + `THRUM_NAME=test_coordinator_main`) `thrum context show --session` returns the marker scenario 17 saved. Round-trip smoke for the save→show contract at the CLI layer, independent of any claude session (migrates § 9.2). |
...
| `20-load-context-slash` | The `/thrum:load-context` slash command sent to the COORD pane causes claude to invoke `Bash` tool with `command: "thrum prime"` (the literal contract spelled out by `claude-plugin/commands/load-context.md`). Asserts the assistant tool_use entry via JSONL polling. Splits the `/` keystroke discretely (mirrors drive.sh's `!`-prefix split rationale) so the slash mode-switch lands cleanly (migrates § 9.4). |
| `21-context-persists-restart` | Save a marker into IMPL's context store via `thrum context save --file`, restart IMPL via `thrum tmux restart impl --force`, wait for new SessionStart, then assert the marker is still readable via `thrum context show --session`. Pins agent-identity-keyed (not session-scoped) persistence; covers the post-restart context-recovery promise § 9.5 documents (migrates § 9.5). |
