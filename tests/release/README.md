...
| `74-restart-coord-fixture-snapshot-file` | Asserts the snapshot scenario 73 staged has the canonical "# Restart Snapshot — `<agent>`" header. Reads from the staged /tmp copy (the original is gone by now — consumed by the new prime). Same shape as scenario 09's `save-header` assertion but against the daemon-side restart-RPC write path (migrates § 10.5). **Depends on scenarios 70 + 73.** |
