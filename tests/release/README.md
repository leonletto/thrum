...
| `69-multi-runtime-restart-force` | Per-scenario detached worktree (`test-force-restart`) + agent-registered managed session (`force_agent_<RUNID>`, role tester). After launch+settle, `thrum tmux restart force-restart-test --force` exits 0 and `thrum tmux status --json` reports state=alive within 10s — pins the `--force` skip-graceful-shutdown contract (migrates § 10C.10). |
