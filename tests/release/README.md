...
| `92-monitor-start` | Sets up the shared monitor fixture used by 92-98: starts a `tail -F`-backed monitor (`kafm11-test-alerts-${RUNID}`) wired to regex `ALERT` + target `@test_coordinator_main` against the run-level daemon, parses the human "Started monitor … (mon_<ulid>) — target …" stdout for the assigned ID (`monitor start` ignores `--json` — see below), and polls `monitor show --json` for status="running" to pin the persistence side of the start contract. `--debounce 30s` (the documented floor) so scenario 95's leading-edge delivery completes inside its time budget. Exports `KAFM11_MON_ID`, `KAFM11_MON_NAME`, `KAFM11_LOG_FILE` for scenarios 93-98 (migrates § 10F.2). **Scenarios 93-98 depend on these exports; helpers/teardown.sh removes the temp log file under /tmp on partial failure.** |
| `93-monitor-list` | Read-only `monitor list --json` projection: filters the response by `KAFM11_MON_ID` and pins the (name, status, target) tuple — name matches the RUNID-anchored fixture name, status="running", target="@test_coordinator_main" (the @-prefix is preserved through the `Mentions[]` round-trip per `internal/daemon/monitor/delivery.go:60-62`). Defensive `(if type=="array" then . else .result // [] end)` shape guard handles both EmitJSON paths (bare array vs. wrapped `{result:[...], hints:[...]}`) (migrates § 10F.3). **Depends on scenario 92.** |
| `94-monitor-show` | Read-only `monitor show <id> --json` per-row projection: pins the four fields the start RPC accepted — name, match pattern ("ALERT"), target, status — round-tripping through the daemon's GetByID path (distinct from list's enumerate path). Single jq pass + `@tsv` → `read` extracts all four fields; emits one assertion per field for attributable failure (migrates § 10F.4). **Depends on scenario 92.** |
| `95-monitor-captures-and-delivers` | End-to-end output-capture + delivery: drains test_coordinator_main's inbox, writes a non-matching "INFO: ..." line to the tailed log file (asserts 0 inbox arrivals carrying the INFO marker — pins the regex-suppression contract), then writes a matching "ALERT: ... kafm11-deliver-marker-${RUNID}" line and polls (up to 60s, 3s steps) for an inbox message containing the marker. Two assertions on the positive path: the marker arrives, AND the delivered message's `.agent_id == "monitor:${KAFM11_MON_NAME}"` (per `internal/daemon/monitor/delivery.go:57`'s caller-id stamp). Drains coord's inbox at end so subsequent scenarios see a clean unread state (migrates § 10F.5). **Depends on scenario 92.** |
| `96-monitor-stop` | `monitor stop <id>` stdout contains "Stopped monitor &lt;id&gt;", the default running-only `monitor list --json` no longer contains the stopped row, AND `monitor list --all --json` retains the row with status="stopped". (`monitor stop` ignores `--json` — see CLI-shape note below.) The third assertion is load-bearing for scenario 97 — restart needs the row persisted, not deleted. The CLI help text says "Stop and remove" but the supervisor calls `store.MarkStopped` (not `store.Delete`) — see `internal/daemon/monitor/supervisor.go:297`; the `--all` visibility check defends against a regression that flipped to genuine deletion (which would silently break restart) (migrates § 10F.6). **Depends on scenarios 92 + 95.** |
| `97-monitor-restart` | `monitor restart <id>` stdout "Restarted — ID: &lt;id&gt;" carries the SAME id we passed in (preserves ID — distinct from a stop+add cycle which would allocate a fresh ID; per `supervisor.Restart` design, `internal/daemon/monitor/supervisor.go:310-358`); after restart the row is back in the running-only `monitor list --json` with status="running" (poll up to 10s — Restart's launch path is async). (`monitor restart` ignores `--json` — see CLI-shape note below.) ID-preservation matters because downstream subscribers (HandleLogs queries, scope filters keyed on `{type:"monitor", value:<name>}`) are stable across restart (migrates § 10F.7). **Depends on scenarios 92 + 96.** |
| `98-monitor-logs` | History-lookup contract for `monitor logs <id>`: stdout non-empty (RPC + render didn't error); contains scenario 95's delivered marker (proves the messages-table projection on `agent_id="monitor:<name>"` survives the stop+restart cycle in 96/97 — same caller-id key persists across the row's lifecycle); `--limit 1` caps the rendered output to ≤1 timestamp-prefixed line (the cap-honored contract — using `--limit 1` makes the cap unambiguous regardless of how many historical matches exist). (`monitor logs` ignores `--json` — see CLI-shape note below.) Each rendered match is one `<RFC3339>  <content>` line per `internal/cli/monitor.go:251-254` (migrates § 10F.8). **Depends on scenarios 92 + 95 + 97.** § 10F.1 (optional setup) and § 10F.9 (cleanup) absorbed: setup is implicit (monitors don't need a tmux session); cleanup is handled by `helpers/teardown.sh`'s defensive log-file sweep + the daemon-stop reaping any monitor child the test left behind.

| `99-tmux-start-banner` | Pins the daemon-side identity banner (thrum-6hqy) emitted by `thrum tmux start` / launch flows. Two assertions against the COORD pane that setup-repo.sh's `thrum tmux start --name coord` produced: (1) pane scrollback contains `Agent: @test_coordinator_main` AND `Role:  coordinator` (banner header lines emitted before the runtime takes over the screen, captured via `tmux capture-pane -S -1000 -p`), AND (2) zero `<command-name>/thrum:prime</command-name>` entries in the coord agent's JSONL — claude has `HasSessionStartHook=true` on its runtime preset, so the daemon's HandleLaunch goroutine should skip the post-launch /thrum:prime send (xupf+2qe2 made it redundant; thrum-6hqy gates it on the preset flag). Read-only, no fixture mutation. |
| `100-tmux-restart-banner` | Pins the same identity-banner contract for `thrum tmux restart` (thrum-6hqy). Per-scenario sub-fixture (worktree `hqy-100-restart-wt` + branch-backed inline-registered agent `hqy_100_restart_agent` + shell runtime — claude would burn API budget on the restart-snapshot preamble work for no contract gain). Drives `thrum tmux restart ${RST_SESSION} --force` from COORD_PANE, asserts the canonical "restarted" success-line, then polls the post-restart pane (`tmux capture-pane -S -1000 -p`) for the banner's `Agent: @${RST_AGENT}` line within 10s. Per-scenario teardown of session+worktree at end (mirrors scenario 91 pattern). |

**CLI-shape note (kafm.11 batch).** `thrum monitor start`, `monitor stop`, `monitor restart`, and `monitor logs` all define the persistent `--json` global flag (visible in `-h`) but their `RunE` handlers in `cmd/thrum/main.go` do not honor it — they always emit human-pretty stdout. `monitor list` and `monitor show` DO honor `--json` (handled via `cli.EmitJSON`). Scenarios 92/96/97/98 therefore parse stdout text for the start/stop/restart/logs assertions and use `capture_thrum_json` only for the read-side `list`/`show` projections. The CLI bug is filed as a follow-up; for the test contract, the stdout shape (`Started monitor … (mon_<ulid>) — target …` / `Stopped monitor <id>` / `Restarted — ID: <id>` / `<RFC3339>  <content>`) IS the user-visible API and is worth pinning.
