...
**Epic thrum-fycc (Daemon Resilience Primitives) is fully closed** — 10/10 tasks.
...
connection. Fixed by collecting all member rows first, closing the cursor,
then resolving roles.
...
- **Fixed:** PreCompact hook now runs `pre-compact-save-context.sh` (was wasted `thrum prime`)
...
- **Original Bug Report**: `dev-docs/bug_reports/eof-on-send-during-daemon-restart.md`
