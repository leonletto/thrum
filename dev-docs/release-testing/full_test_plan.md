...
# Send a broadcast (coordinator is a member of @everyone)
THRUM_NAME=test_coordinator thrum send "Self-exclude test" --to @everyone

# Wait should NOT return the message we just sent
timeout 5 bash -c 'THRUM_NAME=test_coordinator thrum wait --after -10s --timeout 3s 2>&1'; echo "Exit: $?"
# Expected: Exit 1 (timeout) — NOT the broadcast we just sent
# Previously: Wait returned the agent's own outbound messages, causing
# the message-listener to re-trigger on its own replies
```

### J13. Name≠Role Validation (thrum-8mkl)
...
ssh leondev 'export PATH="$HOME/.local/bin:$PATH" && cd /Users/leon/dev/testing/littleCADev && thrum quickstart --name remote_tester --role tester --module smoke --intent "Remote smoke test" && thrum prime'
# Expected: Full prime output — agent registered, session started

ssh leondev 'export PATH="$HOME/.local/bin:$PATH" && cd /Users/leon/dev/testing/littleCADev && thrum send "Hello" --to @remote_tester && thrum inbox --json'
...
```bash
ssh leondev 'eval "$(/opt/homebrew/bin/brew shellenv)" && cd /Users/leon/dev/testing/littleCADev && rm -rf .thrum 2>/dev/null && thrum init && thrum daemon start && thrum quickstart --name brew_tester --role tester --module brew --intent "Brew smoke test" && thrum prime'
# Expected: Full prime output
...
| J10                                               | Name-only routing / auto role groups      | ☐      | thrum-8mkl                          |
| J11                                               | Group-send warning excludes @everyone     | ☐      | thrum-fsvi                          |
| J12                                               | Wait excludes own outbound messages       | ☐      | exclude_self in wait                |
| J13                                               | Name≠role validation                      | ☐      | thrum-8mkl                          |
| **Part K: MCP Routing Parity**                    |                                           |        |                                     |
