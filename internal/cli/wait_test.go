...
func TestWait_ScopeExcludesAgentFilters(t *testing.T) {
	// When --scope is provided, for_agent and for_agent_role must NOT be sent to
	// message.list. AND-ing those params with the explicit scope filter causes
	// scoped messages to be silently dropped.
	//
	...
	// for_agent and for_agent_role must NOT be present when scope is set
...
func TestWait_MentionIncludesAgentFilters(t *testing.T) {
	socketPath := filepath.Join(os.TempDir(), fmt.Sprintf("thrum-mmf-%d.sock", time.Now().UnixNano()%100000))
	_ = os.Remove(socketPath)
	t.Cleanup(func() { _ = os.Remove(socketPath) })
	...
	if forAgent, ok := listParams["for_agent"].(string); !ok || forAgent != "reviewer_main" {
		t.Errorf("Expected for_agent 'reviewer_main', got %v", listParams["for_agent"])
	}
	if forAgentRole, ok := listParams["for_agent_role"].(string); !ok || forAgentRole != "reviewer" {
		t.Errorf("Expected for_agent_role 'reviewer', got %v", listParams["for_agent_role"])
	}
}
