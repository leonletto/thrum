...
// GenerateDaemonID generates a new random ULID-based daemon id.
// Format: "d_" + ulid().
// Daemon ids are per-repo, generated once at thrum init and persisted in
// config.json. Old hostname-derived ids are migrated on daemon startup by
// checking IsLegacyDaemonID.
func GenerateDaemonID() string {
	return "d_" + generateULID()
}

// legacyHostnameDerivedID recomputes the pre-2026-04-17 hostname-derived
// daemon id for a given hostname. Only used by IsLegacyDaemonID to detect
// ids that need migration.
func legacyHostnameDerivedID(hostname string) string {
	if hostname == "" {
		return ""
	}
...
// IsLegacyDaemonID reports whether id was generated by the pre-2026-04-17
// hostname-derivation scheme for the given hostname. Returns false for
// empty ids (those are "missing", not "legacy") and for any value that
// doesn't match the hostname-derived form.
func IsLegacyDaemonID(id, hostname string) bool {
	if id == "" {
		return false
	}
	return id == legacyHostnameDerivedID(hostname)
}
