---
description:
  Shared snapshot-composition protocol consumed by /thrum:restart,
  /thrum:restart-extended, /thrum:sleep, /thrum:sleep-extended. Not
  user-invocable directly.
---

# Snapshot Composition Protocol (shared partial)

Used by: `/thrum:restart`, `/thrum:restart-extended`, `/thrum:sleep`,
...
- **`/thrum:restart` and `/thrum:sleep`** → use the STANDARD 11-section
  structure below.
...
After writing, return control to your variant skill (`/thrum:restart`,
`/thrum:restart-extended`, `/thrum:sleep`, or `/thrum:sleep-extended`) for the
terminal action.
