...
When `thrum prime` detects an active identity, run a **background Cursor Agent**
(or delegated subagent) focused on message listening (~90 min, 6 cycles × 15
min). That agent should call `thrum wait` (blocking) and surface work when
messages arrive. Re-arm after processing. See `references/LISTENER_PATTERN.md` and
