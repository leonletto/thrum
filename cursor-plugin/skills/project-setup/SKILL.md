...
Also needed from AGENTS.md, CLAUDE.md, or conversation context:
...
the `worktree` field). Also check the AGENTS.md or CLAUDE.md worktree table for status info
(active/idle/merged).

### Step 2: Present Worktree Options

For each epic, ask the user in chat with clear multiple-choice options to choose a worktree:
