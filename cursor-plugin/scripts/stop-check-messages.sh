...
# If no unread messages, exit 0 to allow stop normally (Claude Code; Cursor has no equivalent hook).
#
...
PROJECT_DIR="${CURSOR_WORKSPACE_ROOT:-${CLAUDE_PROJECT_DIR:-.}}"
...
# If the listener has died (stale heartbeat or missing), prompt the agent to restart it.
...
# No unread messages and listener is healthy — allow stop
exit 0
