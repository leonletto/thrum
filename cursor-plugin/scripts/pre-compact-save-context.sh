...
# recover context after compaction. A narrative summary (update-context / thrum) is richer;
# this preserves mechanical state at minimum.
#
# Bundled with the Thrum Cursor plugin (ported from Claude Code); run manually or wire if your environment supports it.
...
Run \`thrum context show\` or the plugin **load-context** command for a richer narrative summary.
...
# Informational line when run as a hook; Cursor does not run this automatically.
echo "Pre-compact: saved state to thrum context + ${BACKUP_FILE}. Recover with \`thrum context show\` or the load-context plugin command."
