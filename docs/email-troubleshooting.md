...
`thrum email init`). Re-run init to overwrite the file (or edit it directly,
mode must stay `0600`):

```bash
thrum email init --provider <gmail|fastmail|icloud|custom> --non-interactive \
  --password "<app password>" \
  --daemon-handle <your handle> \
  --target-user <your username> \
  --target-email <your contact email>
```
...
without an explicit error. Check system network logs and verify the IMAP host is
reachable from the daemon's machine.

### Check 4 — Mailbox is the correct folder

By default the bridge watches the `INBOX`. If mail is being delivered to a
different folder (e.g. by a server-side filter), messages are missed. Verify no
server-side filters redirect Thrum messages out of `INBOX`.
...
queue but never sent. See the
[Send Returns over_size_limit](#send-returns-over_size_limit) section below.
...
Both sides must run `thrum email pair --to <handle>` for the pair to complete
(`<handle>` is the other daemon's `--daemon-handle` value). If only one side has
confirmed, the entry remains pending on the unconfirmed side.
`thrum email status` reports the pending stranger-pair count:

```bash
thrum email status
```

The other operator must run their `thrum email pair --to <handle>` command to
confirm.

### Check 3 — Pending pair expired

Pairing requests expire after `pair_pending_ttl_hours` (default: 24 hours). If
the confirmation was not received in time, both sides must re-run
`thrum email pair`.
...
> 25 MB; Fastmail's is 50 MB. Most Thrum coordination messages are well under 1
> MB.
...
File an issue at
[github.com/leonletto/thrum](https://github.com/leonletto/thrum/issues) with the
log snapshot and `thrum email status --json` output.
