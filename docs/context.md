...
Context files are stored in `.thrum/context/{agent-name}.md`:

```
.thrum/
├── context/
│   ├── furiosa.md         # Named agent context
│   ├── maximus.md         # Another named agent
...
When you delete an agent with `thrum agent delete`, the context file is removed
alongside the identity and message files:
