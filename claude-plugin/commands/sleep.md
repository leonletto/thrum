...
Step 5 — the snapshot on disk is the load-bearing artifact, not the status
...
The operator brings you back later by running `thrum tmux create <session-name>`
(or analogous). On runtime start, `thrum prime` auto-injects the snapshot at
