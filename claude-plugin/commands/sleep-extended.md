...
description:
  ...
  Use for designer/architect-grade work where wake may be cold and must
  recover wire contracts + capability matrix + design rationale.
---
...
If `tmux_session` is empty: ABORT before writing any snapshot. No status change, no session end. Exit code 1.
...
**Use the EXTENDED 16-section structure.** The structure block is `§1.` through `§16.` with per-section guidance documented in the partial.

**Note on §3 framing:** For sleep snapshots, §3 frames as "where work stands at park time" rather than "what shipped" — the agent is parking, not completing. Composition discipline (1–3 sentences, specific, load-bearing-first) is identical to restart-extended.
...
If `thrum agent set-status` returns an error, continue to Step 5 — the snapshot on disk is the load-bearing artifact.
...
This emits `agent.session.end` cleanly so the dead-agent sweeper does NOT treat the disconnect as a crash.
...
Tmux pane terminates → runtime process exits → no further activity until operator wakes the agent.

## How wake works

The operator brings you back via `thrum tmux create <session-name>`. On runtime start, `thrum prime` auto-injects the snapshot — same mechanism as restart. Resume from §16 immediate-next-actions.

The snapshot file moves to `.thrum/agents/<your-agent-id>/sessions/` archive on wake (same as restart).
