...
**The trap:** a double-quoted body is processed by your shell *before* thrum
...
`EOF`) disables *all* shell interpretation, so backticks, `$`, apostrophes, and
...
- `--stdin` and `--body-file` are mutually exclusive, and neither may be combined
  with a positional body (the command errors on ambiguity).
